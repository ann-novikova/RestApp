...
from .models import Booking, RestaurantInfo, Table
...
class BookingTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            email="testuser@mail.ru", password="password123"
        )
    ...
    def test_booking_outside_opening_hours(self):
        ...
        early_start = timezone.make_aware(
            datetime.combine(self.tomorrow, datetime.strptime("09:00", "%H:%M").time())
        )
        early_end = early_start + timedelta(hours=1)

        booking = Booking(
            table=self.table, start_time=early_start, end_time=early_end, guests_count=2
        )
        with self.assertRaises(Exception):
            booking.full_clean()
