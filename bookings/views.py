...
class TableAvailabilityView(ListAPIView):
    ...
    def get(self, request):
        ...
        try:
            table_data = BookingService.get_table_availability(date_str, start_str, end_str)
            return Response({'tables': table_data})

        except ValidationError as e:
            return Response({"errors": e.message_dict or e.messages}, status=status.HTTP_400_BAD_REQUEST)
        except Exception as e:
            print(f"Error in TableAvailabilityView: {e}")
            return Response({"error": "Произошла внутренняя ошибка при получении данных о столиках"},
